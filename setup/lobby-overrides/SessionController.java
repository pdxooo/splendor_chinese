...
public class SessionController {
    ...
    /**
     * Removes a session. Its creator can delete it before or after launch. Privileged service accounts retain their
     * existing administrative cleanup permission.
     *
     * @param sessionid the room number
     * @param principal authenticated caller
     * @return deletion result
     * @throws RegistryException if a launched game server cannot be notified
     */
    @PreAuthorize("isAuthenticated()")
    @DeleteMapping("/api/sessions/{sessionid}")
    public ResponseEntity removeSession(@PathVariable long sessionid, Principal principal) throws RegistryException {
        ...
        boolean isCreator = session.getCreator().equals(principal.getName());
        String callerRole = tokenController.currentUserRole().toString();
        boolean isPrivileged = callerRole.contains("ADMIN") || callerRole.contains("SERVICE");

        // Server-side authorization is authoritative; hiding a browser button is not sufficient protection.
        if (!isCreator && !isPrivileged)
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body("Sessions can only be deleted by their creator or an authorized administrator.");

        if (session.isLaunched() && !isCreator && callerRole.contains("SERVICE"))
            if(!principal.getName().equals(gameServers.getRegistringServiceAccountForGame(session.getGameName())))
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body("Launched sessions can only be deleted by the associated service or an admin.");

        // Launched games can only be terminated by the admin who registered the gameserver.
        if (session.isLaunched() && !isCreator && callerRole.contains("ADMIN"))
            if (!principal.getName().equals(gameServers.getRegistringServiceAccountForGame(session.getGameName())))
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Launched sessions can only be deleted by "
